...
# This file was *autogenerated* from the file texloopscript.sage
from sage.all_cmdline import *   # import sage library

_sage_const_400 = Integer(400); _sage_const_0 = Integer(0); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_4 = Integer(4); _sage_const_99 = Integer(99); _sage_const_3 = Integer(3); _sage_const_9 = Integer(9)
...
PARALLEL_CUTOFF = _sage_const_400 
        ...
        for edge in self.comp.edges():
            ...
            for component in self.components:
                for i in component:
                    ...
                    src = edge[_sage_const_0 ]
                    tar = edge[_sage_const_1 ]
                    self.comp[src][tar]['diffweight'] = self.comp[src][tar]['diffweight'].subs({gens[i-_sage_const_1 ]:gens[setting_var]})
                    self.comp[src][tar]['diffweight'] = self.comp[src][tar]['diffweight'].subs({gens[i+size-_sage_const_1 ]:_sage_const_1 })
        ...
        for edge in self.comp.edges():
            ...
            for component in self.components:
                for i in component:
                    ...
                    if i == component[_sage_const_0 ]: continue
                    setting_var = component[_sage_const_0 ] - _sage_const_1 
                    src = edge[_sage_const_0 ]
                    tar = edge[_sage_const_1 ]
                    self.comp[src][tar]['diffweight'] = self.comp[src][tar]['diffweight'].subs({gens[i-_sage_const_1 ]:gens[setting_var]})
                    self.comp[src][tar]['diffweight'] = self.comp[src][tar]['diffweight'].subs({gens[i+size-_sage_const_1 ]:gens[size+setting_var]})
...
#         return
    ...
    # Searches the complex for edges with weight 0 and removes the edge
        ...
        for x,y in elist:
            ...
            if self.comp[x][y]['diffweight'] == _sage_const_0 :
    ...
    def graph_red_search(self, started = False, timerstart = None): 
        ...
        print(len([source for source, target, weight in self.comp.edges(data = 'diffweight') if weight == _sage_const_1 ]))
        while True:
#             count = (count + 1)%
            try:
                red_target = next((source, target) for source, target, weight in self.comp.edges(data = 'diffweight') if weight == _sage_const_1 )
#                 print(self.comp.edges[red_target])
                self.graph_reduction(red_target[_sage_const_0 ], red_target[_sage_const_1 ])
                continue
            except:
                ("StopIteration")
            break
    ...
    def grade_link_complex(self):
        ...
        if self.sigo == None:
            ...
            gridO = list(self.comp.nodes())[_sage_const_0 ]
...
#             self.comp.nodes()[str(gridX)][f'VGrading{i}'] = 0
        ...
        for edge in eds:
            ...
            if edge in self.comp.edges():
                ...
                spanset.append((edge[_sage_const_1 ],edge[_sage_const_0 ]))
        ...
        for vert in self.comp.nodes():
            ...
            for i in range(len(self.components)):
                ...
                self.comp.nodes()[vert][f'AGrading{i}'] = (_sage_const_1 /_sage_const_2 )*(self.comp.nodes()[vert][f'VGrading{i}']-self.comp.nodes()[vert][f'UGrading{i}'])-(_sage_const_1 /_sage_const_2 )*(stab_count - _sage_const_1 )
...
#             return("!!! Unknown number of components for export !!!")
        ...
        if component_length == _sage_const_0 :
        ...
        for node in nxG.nodes():
            ...
            try:
                ...
                nxG.nodes[node]['UGrading'] = int(-_sage_const_99 )
                nxG.nodes[node]['VGrading'] = int(-_sage_const_99 )
                nxG.nodes[node]['AGrading'] = int(-_sage_const_99 )
            for i in range(component_length):
                try:
                    ...
                    nxG.nodes[node][f'AGrading{i}'] = int(-_sage_const_99 )
                    nxG.nodes[node][f'UGrading{i}'] = int(-_sage_const_99 )
                    nxG.nodes[node][f'VGrading{i}'] = int(-_sage_const_99 )
        ...
        for i in range(len(self.components)):
            for vert in self.comp:
                ...
                if self.comp.nodes()[vert][f"AGrading{i}"] >= grading_cutoff[i]:
                    self.comp.nodes()[vert][f"AGrading{i}"] += _sage_const_1 
                    self.comp.nodes()[vert][f"UGrading{i}"] += -_sage_const_2 
                    ...
                    for pred in self.comp.predecessors(vert):
                        ...
                        self.comp[pred][vert]['diffweight'] = self.comp[pred][vert]['diffweight']*(generators[i]**(-_sage_const_1 ))
    ...
    def surgery(self, grading_list = None, target_grading = None):
        ...
        for i in range(len(target_grading)):
            ...
            grading_ranges.append(list(range(target_grading[i], grading_list[i]+_sage_const_1 )))
    ...
    def relative_grading_loop(self, grading_key, base_vertex, fn1, fn2, span = None, grading_multiplicity = _sage_const_1 ):
        ...
        self.comp.nodes()[str(base_vertex)][f'{grading_key}'] = _sage_const_0 

        on_deck = [str(base_vertex)]
        
        in_the_hole = []

        while len(on_deck) > _sage_const_0 : 
    ...
    def componentwise_relative_grading_loop(self, grading_key, base_vertex, fn1, fn2, span = None, grading_multiplicity = _sage_const_1 ):
        ...
        for i in range(grading_multiplicity):
            ...
            self.comp.nodes()[str(base_vertex)][f'{grading_key}{i}'] = _sage_const_0 
...
#########################################
    ...
    def virtual_U_gradings_pred(self, i, pred, vert, component_columns):
        ...
        self.comp.nodes()[pred][f'UGrading{i}'] = self.comp.nodes()[vert][f'UGrading{i}'] + _sage_const_1  - _sage_const_2 *Upows
    ...
    def virtual_U_gradings_succ(self, i, succ, vert, component_columns):        
        ...
        Upows = link_U_deg(ed_weight, self.ring, component_columns)
        self.comp.nodes()[succ][f'UGrading{i}'] = self.comp.nodes()[vert][f'UGrading{i}'] - _sage_const_1  + _sage_const_2 *Upows
    ...
    def virtual_V_gradings_pred(self, i, pred, vert, component_columns):
        ...
        self.comp.nodes()[pred][f'VGrading{i}'] = self.comp.nodes()[vert][f'VGrading{i}'] + _sage_const_1  - _sage_const_2 *Vpows
    ...
    def virtual_V_gradings_succ(self, i, succ, vert, component_columns):        
        ...
        Vpows = link_V_deg(ed_weight, self.ring, component_columns)
        self.comp.nodes()[succ][f'VGrading{i}'] = self.comp.nodes()[vert][f'VGrading{i}'] - _sage_const_1  + _sage_const_2 *Vpows
    ...
    def maslov_U_pred(self, pred, vert):
        ...
        Upows = link_U_deg(ed_weight, self.ring, component_columns)
        self.comp.nodes()[pred]['UGrading'] = self.comp.nodes()[vert]['UGrading'] + _sage_const_1  - _sage_const_2 *Upows        
    ...
    def maslov_U_succ(self, succ, vert):
        ...
        Upows = link_U_deg(ed_weight, self.ring, component_columns)
        self.comp.nodes()[succ]['UGrading'] = self.comp.nodes()[vert]['UGrading'] - _sage_const_1  + _sage_const_2 *Upows
    ...
    def parallel_reduction_helper(self, subgraph_set, overwrite = True):
        ...
        for count, subgraph in enumerate(subgraph_set):
            ...
            process_dict[count] = mp.Process(target = subgraph[_sage_const_0 ].range_graph_red_search(), args = subgraph[_sage_const_1 ] )
        ...
        result = subgraph_set[_sage_const_0 ][_sage_const_0 ]
    ...
    def ego_parallel_red_search(self, cutoff = PARALLEL_CUTOFF, proc_count = PROCESSOR_COUNT):
        ...
        while len([source for source, target, weight in self.comp.edges(data = 'diffweight') if weight == _sage_const_1 ]) > cutoff:
               print(str(len([source for source, target, weight in self.comp.edges(data = 'diffweight') if weight == _sage_const_1 ])) + "reducible edges remaining")
               reducible_edge = rd.sample([source for source, target, weight in self.comp.edges(data = 'diffweight') if weight == _sage_const_1 ], _sage_const_1 )
               
               self.ego_parallel_sweep(reducible_edge[_sage_const_0 ], proc_count)
    ...
    def ego_parallel_sweep(self, start_vert = None, proc_count = PROCESSOR_COUNT):
        ...
        if start_vert == None:
            ...
            start_vert = self.comp.nodes()[_sage_const_0 ]
        
        size = len(list(self.comp.nodes)[_sage_const_0 ])
#         (self.comp.nodes[0])
...
#                         processed_subgraphs = []
            ...
            result = processed_subgraphs[_sage_const_0 ].comp
...
def subgraph_red_search(subg, search_reg, result_list):
    ...
    for ed in [edge for edge in search_region.comp.edges() if search_region.comp.edges[edge]['diffweight'] == _sage_const_1 ]:
...
#             print("reducing an edge")
            if (subgraph.comp.edges()[ed]['diffweight'] == _sage_const_1 ):
                subgraph.graph_reduction(ed[_sage_const_0 ], ed[_sage_const_1 ])
...
def ego_split(graph, vertex, n):
    ...
    safety.remove_nodes_from(result[n - _sage_const_1 ].nodes())
        
    for i in range(n-_sage_const_1 , _sage_const_0 , -_sage_const_1 ):
        
        result[i].remove_nodes_from(result[i-_sage_const_1 ].nodes())
...
def ego_region_partition(n):
    ...
    result["block0"] = {"search_region": [_sage_const_0 ,_sage_const_1 ] , "reserved_region": [_sage_const_2 ]}
    
    for i in range(_sage_const_1 ,split_count):
        
        result[f"block{i}"] = {"search_region" : [_sage_const_3 *i, _sage_const_3 *i+_sage_const_1 ], "reserved_region" : [_sage_const_3 *i-_sage_const_1 ,_sage_const_3 *i+_sage_const_2 ]}
...
def partition_block_iterator(blocks, step_size):
    ...
    for count, block in enumerate(blocks):
        ...
        if count == _sage_const_0 :
            ...
            for i in range(_sage_const_1 , len(block)):
...
def construct_cinf(g, sigx, sigo, size = -_sage_const_1 ): 
    ...
    if size == -_sage_const_1 :
        ...
        print("Grid size is " + str(size/_sage_const_2 ))
        n = size/_sage_const_2                                                                               #into a list and checking the length of#the weight of the first edge
    else:
        n = size
    timerstart = time.time()
    F,Vars = cinf_coeff(n)
    ...
    for edge in g.edges:
        ...
        start = edge[_sage_const_0 ]
        end = edge[_sage_const_1 ]
        poly = F(_sage_const_0 )
        
        
        for subweight in g[edge[_sage_const_0 ]][edge[_sage_const_1 ]]['diffweight']:
            
            i = _sage_const_0 
            polychange = F(_sage_const_1 )
...
def cinf_coeff(size):
    ...
    F = LaurentPolynomialRing(GF(_sage_const_2 ), varis)
    F.inject_variables()
...
def range_skip_entry(n, skip):
    ...
    for j in range(skip+_sage_const_1 , n): u.append(j)       
    return u
...
def link_U_deg(poly, ring, component_columns):
    ...
    if type(poly) == sage.rings.finite_rings.integer_mod.IntegerMod_int: return _sage_const_0 
...
#         raise Exception("Ran into a non-homogoneous degree change - polynomial wasn't a monomial")
    ...
    for i in component_columns:
        ...
        degree = degree + powers[i-_sage_const_1 ]
...
def degree_partition(max_grading_step, lower_range, upper_range, split_count):
    ...
    active_range = block_size - _sage_const_2 *max_grading_step
    print("active range " + str(active_range))
    
    if ((active_range <= _sage_const_0 ) and (split_count > _sage_const_1 )) :
        
        print(str((max_grading_step, lower_range, upper_range, split_count - _sage_const_1 )))
        print("Cannot partition the graph into this many pieces! Parititioning into a smaller number of pieces")
        return degree_partition(max_grading_step, lower_range, upper_range, split_count - _sage_const_1 )
    
    if split_count == _sage_const_1 :
        
        return None
    
    block = []
    
    max_grading_step += -_sage_const_1 
    
    trailing_edge = lower_range - _sage_const_1  - max_grading_step
...
for knot in full_dict:
    if os.path.isfile("tex" + knot + ".txt"): 
        ...
        continue
    if len(full_dict[knot][_sage_const_0 ]) < _sage_const_9 :
        ...
        ssigx = str(full_dict[knot][_sage_const_0 ])
        ssigo = str(full_dict[knot][_sage_const_1 ])
