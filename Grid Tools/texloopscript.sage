...
for knot in full_dict:
    if os.path.isfile("tex" + knot + ".txt"): 
        print("Knot or link " + knot + " tex file already exists - skipping" )
        continue
    if len(full_dict[knot][0]) < 9:
        f = open(f'tex{knot}.txt', 'w')
        comp = link_GFC(*full_dict[knot], knot)
        npoly = comp.newton_poly()
        print(npoly)
        npolyfact = npoly.factor()
        print(npolyfact)
        keys = ["UGrading", "VGrading", "AGrading"]
        for i in range(len(comp.components)):
            keys.append(f"AGrading{i}")
        comp.find_grading_ranges()
        comp.find_grading_ranges("UGrading")
        comp.find_grading_ranges("VGrading")
        for key in keys:
            comp.find_grading_ranges(key)
        snpoly = str(npoly)
        snpolyfact = str(npolyfact)
        ssigx = str(full_dict[knot][0])
        ssigo = str(full_dict[knot][1])
        f.write("\\begin{longtable}{p{.25\\textwidth} p{.75\\textwidth}}")
        f.write(f"${knot}$   \\\\ \\hline \n")
        f.write(f"$\\sigma_X$   & ${ssigx}$ \\\\ \n")
        f.write(f"$\\sigma_O$  & ${ssigo}$ \\\\ \n")
        f.write(f"$GFC^\\infty$   \\\\ \n")
        f.write(f"Number of Generators & {len(comp.comp.nodes())} \\\\ \n")
        f.write(f"Number of Edges & {len(comp.comp.edges())} \\\\ \n")
        f.write("\\end{longtable} \n")
        f.write("\\begin{description} \n")
        f.write(f"\\item[Newton Polynomial]   ${snpoly}$ \n")
        f.write(f"\\item[Newton Polynomial (factored)] ${snpolyfact}$ \n")
        f.write("\\end{description} \n")
        f.write("\\begin{longtable}{p{.25\\textwidth} p{.75\\textwidth}} \n")

        f.write(f"Grading Ranges   &  \\\\ \n")
        comp.to_minus()
        comp.ego_parallel_red_search(proc_count = PROCESSOR_COUNT)
        npoly = comp.newton_poly()
        npolyfact = npoly.factor()
        ...
        snpolyfact = str(npolyfact)
        for key in keys:
            f.write(f" & ${comp.min_gradings[key]}<{key}<{comp.max_gradings[key]}$   \\\\ \n")
        f.write("& \\\\ \n")
        f.write(f"$GFC^-$   \\\\ \n")
        f.write(f"Number of Generators & {len(comp.comp.nodes())} \\\\ \n")
        f.write(f"Number of Edges & {len(comp.comp.edges())} \\\\ \n")
